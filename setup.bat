...
set GITHUB_REPO=Wow-Legion-One-Stop-Setup
...
:: ---- WoW client check ----
if not exist "%CLIENT_DIR%\Wow.exe" (
    echo  [!] WoW client not found.
    echo      Please put your WoW 7.3.5 client folder at:
    echo      %CLIENT_DIR%\
    echo      It must contain Wow.exe inside it.
    echo.
    pause & exit /b 1
)
echo  [OK] WoW client found: %CLIENT_DIR%
...
:: Patch DataDir to absolute path
set "DATA_DIR_ESC=%SERVER_DIR:\=\\%\\data"
powershell -Command ^
  "(Get-Content '%SERVER_DIR%\worldserver.conf') ^
   -replace 'DataDir\s*=.*', 'DataDir = \"%SERVER_DIR%\data\"' ^
...
echo      you run start_server.bat and use the in-game worldserver console.

:: ---- Patch WoW client to connect to localhost ----
echo.
echo  [*] Patching WoW client to connect to localhost...
if exist "%SERVER_DIR%\connection_patcher.exe" (
    copy "%SERVER_DIR%\connection_patcher.exe" "%CLIENT_DIR%\" >nul 2>&1
    pushd "%CLIENT_DIR%"
...
echo.
echo   Next steps:
echo   1. Run start_server.bat to launch bnetserver + worldserver
echo   2. Wait ~60 seconds for the world to finish loading
echo   3. Open WoW from:
echo      %CLIENT_DIR%\Wow.exe
echo   4. Log in with:
echo      Username: !ACCT_USER!
echo      Password: !ACCT_PASS!
echo.
echo   To create more accounts later, run create_account.bat
